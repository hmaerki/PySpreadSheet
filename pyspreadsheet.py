...
class Table:
  ...
  def dump(self, f):
    print(file=f)
    print(f'Table: {self.table_name}', file=f)
    print(f'  {self.column_names_text}', file=f)
    print(file=f)
...
class ExcelReader:
      ...
      for rowidx, row in enumerate(worksheet.rows):
        ...
        def get_value(idx):
          ...
          if val is None:
            return ''
          return str(val)
        cell_table = get_value(COLUMN_TABLE)
        cell_name = get_value(COLUMN_NAME)
  ...
  def dump(self, f):
    ...
    # for _, obj_table in self.__dict_tables.items():
    #   obj_table.dump(f)
